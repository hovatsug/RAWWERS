            ...
            child: ConstrainedBox(
              ...
              child: RCard(
                child: Column(
                  ...
                  children: [
                    Text('Pro Register', style: Theme.of(context).textTheme.headlineMedium),
                    ...
                    SizedBox(
                      ...
                      child: RButton(
                        label: 'Create Pro Account',
                        isLoading: state.loading,
                        onPressed: () async {
                          await ref.read(authControllerProvider.notifier).registerAsPro(
                                email: _emailController.text.trim(),
                                password: _passwordController.text,
                              );
                          final next = ref.read(authControllerProvider);
                          if (next.me?.roles.contains('pro') == true) {
                            setState(() => _ok = true);
                          }
                        },
                      ),
                    ...
                    if (_ok) ...[
                      const SizedBox(height: RTokens.spacingX2),
                      RButton(
                        label: 'Open Pro Dashboard',
                        variant: RButtonVariant.secondary,
                        onPressed: () => context.go('/pro/dashboard'),
                      ),
                    ],
                    if (state.error != null) ...[
                      const SizedBox(height: RTokens.spacingX2),
